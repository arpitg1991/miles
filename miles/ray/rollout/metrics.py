...
def _compute_metrics_from_samples(args, samples):
    ...
    # Under the arena plugin TRUNCATED marks only a sample removed for a hard
    # context overflow (bad logprobs are ABORTED, gym-side pads FAILED), so this
    # ratio is that removal fraction. To explain why episodes end, read the
    # plugin's rollout/stop/<reason> fractions.
