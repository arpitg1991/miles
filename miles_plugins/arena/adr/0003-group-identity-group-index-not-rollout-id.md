...
**Amended by:** ADR-0011 (`--arena-train-segments all` stamps the EPISODE id, never the group id, on `rollout_id`; `final` mode keeps this decision. `all` is the flag default since 2026-09-24, so a production single-step batch carries the episode id, which equals the `index` this ADR pins)
