...
the per-token advantages before OPD and whitening. ``Sample.advantage_scale``
is None (all ones) unless a rollout path sets it; the arena plugin sets none.
"""
...
def test_scale_is_a_plain_multiply():
...
def test_compute_advantages_and_returns_applies_the_scale(fixed_advantages):
    rollout_data = _rollout_data([1.0, 0.0, -1.0, -1.0])

    loss_utils.compute_advantages_and_returns(_args(), rollout_data)

    torch.testing.assert_close(rollout_data["advantages"][0], torch.tensor([2.0, 0.0, -0.5, 3.0]))
    torch.testing.assert_close(rollout_data["returns"][0], torch.tensor(_ADV))  # returns untouched


def test_compute_advantages_and_returns_without_scale_is_a_noop(fixed_advantages):
